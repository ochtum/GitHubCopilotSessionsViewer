...
set "LOG_FILE=%TEMP%\github-copilot-sessions-viewer.log"
set "PY_CMD="
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$app='%APP_DIR_WIN%'; $pidFile='%PID_FILE%'; $log='%LOG_FILE%'; $py='%PY_CMD%';" ^
  "$cmd='cd /d \"' + $app + '\" && ' + $py + ' viewer.py > \"' + $log + '\" 2>&1';" ^
  "$p=Start-Process -FilePath 'cmd.exe' -ArgumentList '/c', $cmd -WindowStyle Minimized -PassThru;" ^
